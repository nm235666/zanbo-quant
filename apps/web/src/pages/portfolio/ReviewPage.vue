<template>
  <AppShell title="交易复盘" subtitle="执行后复盘分析：滑点统计、交易评级与经验沉淀。">
    <div class="space-y-4">
      <!-- 复盘列表 -->
      <PageSection title="复盘记录" :subtitle="`共 ${reviews.length} 条`">
        ...
        <div v-else class="divide-y divide-[var(--line)]">
          ...
          >
            ...
            <div v-if="review.review_note" class="mt-2 text-[13px] leading-6 text-[var(--muted)]">
              {{ review.review_note }}
            </div>
          </div>
        </div>
