      ...
      <PageSection :title="`评分结果 (${scores?.total || 0})`" subtitle="点击股票名进入详情页做进一步研究。">
        ...
        <div class="grid gap-3 lg:hidden">
          ...
          >
            ...
            <div class="mt-3 flex flex-wrap gap-2">
              <RouterLink class="rounded-full border border-[var(--line)] bg-white px-3 py-2 text-xs font-semibold text-[var(--brand)]" :to="`/app/data/stocks/detail/${row.ts_code}`">查看详情</RouterLink>
              <RouterLink class="rounded-full border border-[var(--line)] bg-white px-3 py-2 text-xs font-semibold text-[var(--ink)]" :to="`/app/desk/workbench?ts_code=${encodeURIComponent(row.ts_code)}&from=stock_scores`">→工作台</RouterLink>
            </div>
          </InfoCard>
        </div>
        ...
        <DataTable class="hidden lg:block" :columns="columns" :rows="scores?.items || []" row-key="ts_code" empty-text="暂无评分数据">
          <template #cell-name="{ row }">
            <RouterLink class="font-semibold text-[var(--brand)]" :to="`/app/data/stocks/detail/${row.ts_code}`">{{ row.name || row.ts_code }}</RouterLink>
          </template>
...
const workbenchEntryTo = computed(() => {
  ...
  return `/app/desk/workbench${qs ? '?' + qs : ''}`
})
