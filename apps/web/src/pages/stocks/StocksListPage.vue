      ...
      <div class="page-hero-grid">
        <div class="page-hero-card">
          ...
          <div class="page-action-cluster">
            ...
            <RouterLink class="rounded-2xl border border-[var(--line)] bg-white px-4 py-3 text-sm font-semibold text-[var(--ink)]" to="/app/data/stocks/scores">
              打开综合评分
            </RouterLink>
          </div>
        </div>
      ...
      <PageSection :title="`股票结果 (${stocks?.total || 0})`" subtitle="点击代码或名称进入统一股票详情页。">
        ...
        <DataTable class="hidden lg:block" :columns="columns" :rows="stocks?.items || []" row-key="ts_code" empty-text="暂无股票结果">
          <template #cell-ts_code="{ row }">
            <RouterLink class="font-bold text-[var(--brand)]" :to="`/app/data/stocks/detail/${row.ts_code}`">{{ row.ts_code }}</RouterLink>
          </template>
          <template #cell-name="{ row }">
            <RouterLink class="font-semibold" :to="`/app/data/stocks/detail/${row.ts_code}`">{{ row.name }}</RouterLink>
          </template>
