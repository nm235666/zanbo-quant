...
class LlmJobsTest(unittest.TestCase):
    def test_llm_job_target_shape(self):
        ...
        target3 = get_llm_job_target("multi_role_v3_worker_guard")
        self.assertEqual(target3["job_key"], "multi_role_v3_worker_guard")
        self.assertEqual(target3["runner_type"], "multi_role_maintenance")
        self.assertIn("ensure_multi_role_v3_workers", target3["target"])

    def test_run_llm_job_describe(self):
        for job_key, expected_runner in (
            ("llm_provider_nodes_probe", "llm_probe"),
            ("multi_role_v3_worker_guard", "multi_role_maintenance"),
        ):
            proc = subprocess.run(
                [
                    sys.executable,
            ...
            self.assertEqual(payload["runner_type"], expected_runner)
